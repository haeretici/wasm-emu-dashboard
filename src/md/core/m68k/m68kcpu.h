...
extern uint32_t g_last_68k_rom_read_offset;
...
INLINE uint m68ki_read_imm_16(void)
{
#if M68K_EMULATE_PREFETCH
  uint temp_val;
  uint temp_pc = REG_PC; // Capture PC before it advances
  ...
  // --- Custom CDL Code Hook (0x01) START ---
  if (temp_pc < cart.romsize) {
      g_cdl_buffer[temp_pc] |= 0x01;
      g_cdl_buffer[temp_pc + 1] |= 0x01;
  }
  // --- Custom CDL Code Hook (0x01) END ---
  ...
  // --- Custom CDL Code Hook (0x01) START ---
  if (pc < cart.romsize) {
      g_cdl_buffer[pc] |= 0x01;
      g_cdl_buffer[pc + 1] |= 0x01;
  }
  // --- Custom CDL Code Hook (0x01) END ---
...
INLINE uint m68ki_read_imm_32(void)
{
  ...
  if (temp_pc < cart.romsize) {
      g_cdl_buffer[temp_pc] |= 0x01;
      g_cdl_buffer[temp_pc + 1] |= 0x01;
      g_cdl_buffer[temp_pc + 2] |= 0x01;
      g_cdl_buffer[temp_pc + 3] |= 0x01;
  }
  // --- Custom CDL Code Hook (0x01) END ---
  ...
  if (pc < cart.romsize) {
      g_cdl_buffer[pc] |= 0x01;
      g_cdl_buffer[pc + 1] |= 0x01;
      g_cdl_buffer[pc + 2] |= 0x01;
      g_cdl_buffer[pc + 3] |= 0x01;
  }
  // --- Custom CDL Code Hook (0x01) END ---
...
INLINE uint m68ki_read_8(uint address)
{
  ...
  // --- Custom CDL Data Hook (0x02) START ---
  if (address < cart.romsize) {
      g_cdl_buffer[address] |= 0x02; // Tag 1 byte as Data
      g_last_68k_rom_read_offset = address;
  }
  // --- Custom CDL Data Hook (0x02) END ---
...
INLINE uint m68ki_read_16(uint address)
{
  ...
  // --- Custom CDL Data Hook (0x02) START ---
  if (address < cart.romsize) {
      g_cdl_buffer[address] |= 0x02;     // Tag high byte as Data
      g_cdl_buffer[address + 1] |= 0x02; // Tag low byte as Data
      g_last_68k_rom_read_offset = address;
  }
  // --- Custom CDL Data Hook (0x02) END ---
...
INLINE uint m68ki_read_32(uint address)
{
  ...
  if (UNLIKELY(cpu_hook))
    ...
    // --- Custom CDL Data Hook (0x02) START ---
    if (address < cart.romsize) {
        g_cdl_buffer[address] |= 0x02;
        g_cdl_buffer[address + 1] |= 0x02;
        g_cdl_buffer[address + 2] |= 0x02;
        g_cdl_buffer[address + 3] |= 0x02;
        g_last_68k_rom_read_offset = address;
    }
    // --- Custom CDL Data Hook (0x02) END ---
