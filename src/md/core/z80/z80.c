...
INLINE unsigned char cdl_z80_readop(unsigned int a, bool is_code) {
    // 1. Get the actual physical host memory pointer the Z80 is accessing
    extern uint8_t* g_cdl_buffer;
    extern uint32_t g_last_z80_rom_read_offset;
    uint8_t *host_ptr = z80_readmap[a >> 10] + (a & 0x03FF);

    // 2. Check if this pointer falls within the loaded Cartridge ROM buffer.
    // In Genesis Plus GX, the ROM buffer is usually `cart.rom` and its size is `cart.romsize`.
    if (host_ptr >= cart.rom && host_ptr < (cart.rom + cart.romsize)) {

        // 3. Calculate the exact flat ROM offset using pointer subtraction!
        uint32_t rom_offset = (uint32_t)(host_ptr - cart.rom);

        // 4. Tag the CDL buffer
        g_cdl_buffer[rom_offset] |= (is_code ? 0x01 : 0x02);
        g_last_z80_rom_read_offset = rom_offset;
    }

    // Return the actual byte value
    return *host_ptr;
}

// Then your macros remain the same:
#define cpu_readop(a)     cdl_z80_readop(a, true)
#define cpu_readop_arg(a) cdl_z80_readop(a, false)
