   ...
   do
   {
      ...
      do
      {
         ...
         CPU.Cycles += CPU.MemSpeed;
            // --- Custom SNES CDL HOOK (CODE - 0x01) START ---
         // Ensure we are executing from the ROM memory space
         extern uint8_t* g_snes_cdl_buffer; // Defined in exports.c
         extern int g_snes_cdl_size;

         if (g_snes_cdl_buffer != NULL && CPU.PC >= Memory.ROM && CPU.PC < (Memory.ROM + Memory.CalculatedSize)) {
            // Pointer arithmetic yields the exact flat ROM file index!
            uint32_t rom_index = (uint32_t)(CPU.PC - Memory.ROM);
            g_snes_cdl_buffer[rom_index] |= 0x01;
         }
