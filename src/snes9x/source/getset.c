...
uint8_t S9xGetByte(uint32_t Address)
{
   ...
   if (GetAddress >= (uint8_t*) MAP_LAST)
   {
      if (Memory.BlockIsRAM [block])
        ...
        // --- Custom SNES CDL HOOK (DATA - 0x02) START ---
        extern uint8_t* g_snes_cdl_buffer;
        extern uint32_t g_last_rom_read_offset;
        extern bool g_last_read_was_rom;

        uint8_t *host_ptr = GetAddress + (Address & 0xffff);

        if (g_snes_cdl_buffer != NULL) {
            if (host_ptr >= Memory.ROM && host_ptr < (Memory.ROM + Memory.CalculatedSize)) {
                uint32_t rom_index = (uint32_t)(host_ptr - Memory.ROM);
                g_snes_cdl_buffer[rom_index] |= 0x02;

                // Track this payload for potential Audio transfers
                g_last_rom_read_offset = rom_index;
                g_last_read_was_rom = true;
            } else {
                // If the CPU reads from WRAM/SRAM, break the chain to avoid false positives
                g_last_read_was_rom = false;
            }
        }
        // --- Custom SNES CDL HOOK END ---
