...
void S9xSetPPU(uint8_t Byte, uint16_t Address)
{
   if (Address <= 0x2183)
   {
      ...
      case 0x2143:
      case 0x2144:
        // --- Custom SNES Manual Audio Write Hook (0x04) START ---
        extern uint8_t* g_snes_cdl_buffer;
        extern uint32_t g_last_rom_read_offset;
        extern bool g_last_read_was_rom;

        // If the CPU just fetched a byte from the ROM, and the next thing it does
        // is write to the APU, we confidently flag that source byte as Audio Data!
        if (g_snes_cdl_buffer != NULL && g_last_read_was_rom) {
            g_snes_cdl_buffer[g_last_rom_read_offset] |= 0x04;
        }
        // --- Custom SNES Manual Audio Write Hook END ---
      case 0x2145:
      case 0x2146:
      case 0x2147:
      case 0x2148:
