...
void S9xDoDMA(uint8_t Channel)
{
   ...
   if (!d->TransferDirection)
   {
      ...
      // --- Custom SNES CDL HOOK (DMA DATA & AUDIO) START ---
      extern uint8_t* g_snes_cdl_buffer;

      if (g_snes_cdl_buffer != NULL) {
          int32_t current_p = p;

          // Check if the DMA destination (B-Bus) is the APU ports (0x2140 - 0x2143)
          // d->BAddress holds the lower byte of the 0x21XX register.
          uint8_t cdl_flag = (d->BAddress >= 0x40 && d->BAddress <= 0x43) ? 0x04 : 0x02;

          for (int32_t i = 0; i < count; i++) {
              uint8_t *host_ptr = base + current_p;

              // Ensure the pointer is actually inside the loaded ROM buffer
              if (host_ptr >= Memory.ROM && host_ptr < (Memory.ROM + Memory.CalculatedSize)) {
                  uint32_t rom_index = (uint32_t)(host_ptr - Memory.ROM);
                  g_snes_cdl_buffer[rom_index] |= cdl_flag;
              }
              current_p += inc;
          }
      }
